...
class okcoinusd (Exchange):
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if market['future']:
            method += 'Future'
            contract_type = params.get('contract_type')  # this_week, next_week, quarter
            if not contract_type:
                raise ExchangeError(self.id + ' future orders requires a contract_type parameter in params')
            position_type = params.get('type')  # 1: open long position 2: open short position 3:liquidate long position 4: liquidate short position
            if not position_type:
                raise ExchangeError(self.id + ' future orders requires a position_type parameter in params')
            match_price = params.get('match_price', 0)  # match best counter party price (BBO)? 0: No 1: Yes If yes, the 'price' field is ignored
            if match_price == 0 and price is None:
                raise ExchangeError(self.id + ' future limit orders requires amount parameter')
            leverage_rate = params.get('leverage_rate')  # 10, 20
            if leverage_rate:
    ...
    def parse_order(self, order, market=None):
        ...
        # https://github.com/anyex/anyex/issues/2452
    ...
    def parse_position(self, position, market=None):
        ...
        margin_call_price = liquidation_price  # is it?
